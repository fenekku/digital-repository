...
RSpec.describe InvenioRdmRecordConverter do
  ...
  describe "#subjects_for_scheme" do
    ...
    context "tag scheme" do
      let(:tag_terms) { [lcnaf_term] }
      let(:tag_subject_type){ :tag }
      let(:expected_tag_result) { ["id": "http://id.loc.gov/authorities/names/n90699999"] }

      it "returns metadata for term" do
        expect(invenio_rdm_record_converter.send(:subjects_for_scheme, tag_terms, tag_subject_type)).to eq(expected_tag_result)
      end
    end
  end
end
